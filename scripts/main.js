...
searchBtn.onclick=function searchClick(){
 ...
 window.onload=function(){
 	...
 	function Show () {
 		var timer;
 		var count=0;
 		function runCheck (count) {
 			$(".banner>div").each(function() {$(this).removeClass('current')});
 			$(".banner>ul>li").each(function() {$(this).removeClass('current')});
      $(".banner>div").eq(count).addClass("current");
      $(".banner>ul>li").eq(count).addClass("current"); 
 		}
